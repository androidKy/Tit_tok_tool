...
import com.utils.common.backup.BackupConstant;
...
public class ZipUtils {
        ...
        if (sourceFile.isFile()) {
            ...
            if (listFiles == null || listFiles.length == 0) {
                ...
                for (File file : listFiles) {
                    //不对自身压缩进行压缩，否则会无限压缩进入死循环
                    if (file.getName().equals(BackupConstant.ZIP_DIR_NAME)
                            || file.getName().equals("com.xunmeng.pinduoduo")) {
                        continue;
                    }
                    //不对apk文件进行压缩备份
    ...
    /**
     * 解压缩, /sdcard/zipFile/2019_1011_20191011181427.zip: open failed: EACCES (Permission denied)
     *
     * @param zipFile
     * @param targetDir
     */
