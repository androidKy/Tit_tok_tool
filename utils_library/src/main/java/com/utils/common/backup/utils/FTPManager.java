package com.utils.common.backup.utils;/*
package com.buy.together.backup.utils;
...
/**
    ...
    public synchronized boolean connect(String address, String userName, String pwd) {
        boolean bool = false;
        try {
            ...
            if (FTPReply.isPositiveCompletion(ftpClient.getReplyCode())) {
                LogUtil.out(TAG, "ftp连接成功 replyCode = " + ftpClient.getReplyCode());
...
/**
    ...
    public FTPManager setFileName(String fileName) {
...
/**
    ...
    public synchronized void downloadFile(String localPath, String serverPath, IProgressListener progressListener)
        ...
        long startTime = System.currentTimeMillis();
        //创建本地文件夹
        File apkDirPath = new File(localPath);
        if (!apkDirPath.exists()) {
            apkDirPath.mkdirs();
        }
        //先检查本地文件是否存在
        if (!TextUtils.isEmpty(mFileName)) {
            File apkFile = new File(localPath + mFileName);
            if (apkFile.exists()) {
                LogUtil.out(TAG, "本地文件已存在，不重复下载");
                if (progressListener != null) {
